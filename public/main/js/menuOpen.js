...
function saveDataPoint() {
  let data = JSON.stringify({
    filter: JSON.stringify(arrButNum).replaceAll('"', '').replaceAll('[', '').replaceAll(']', ''),
    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      for (let i = 0; i < arrData.length; i += 1) {
        dataInfo_1 +=
          ...
          arrData[i].timework_1 +
          '-' +
          arrData[i].timework_2 +
          '</div>' +
          '</div>' +
          '<div class="bottomPoint">' +
          arrData[i].discription +
          '</div>' +
          '</div>' +
          '</div>' +
...
function addCardLeftMenu() {
  dataInfo_1 +=
    ...
    '</div>' +
    '<div class="middleRight">' +
    document.querySelector('#timeWork_1').value +
    '-' +
    document.querySelector('#timeWork_2').value +
    '</div>' +
    '</div>' +
    '<div class="bottomPoint">' +
